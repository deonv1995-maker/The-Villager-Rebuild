...
const captureConstruction = game => ({
  ...
  builtLogs: game.physicalLogs.builtLogs
    ...
    .map(entry => ({
      ...
      roofForm: entry.roofForm,
      highEdge: entry.highEdge,
...
const restoreConstruction = (game, state) => {
  ...
  for (const saved of builtLogs.sort((left, right) => left.id - right.id)) {
    ...
    const placement = {
      ...
      roofForm: saved.roofForm ?? null,
      highEdge: saved.highEdge ?? null,
    ...
    const built = {
      ...
      roofForm: placement.roofForm,
      highEdge: placement.highEdge,
