 ...
 * sculpts torso-dominant native bind geometry, restores the native shoulder joint
 * centres and calibrates the presentation hand socket after the native rig loads.
 * Arm-dominant geometry deliberately stays authored by Prisma so presentation
 * tuning cannot pull limb vertices away from their skinned bind space.
 */
export class MasculinePrismaHumanoidPresentation extends PrismaRiggedHumanoidPresentation {
  ...
  #applyPresentationProfile() {
    ...
    for (const side of ['left', 'right']) {
      ...
      // Keep the arm on the native authored skeleton. Translating shoulder joints
      // or resculpting arm vertices in a different bind space can create stretched
      // triangles during animation, so continuity is handled by preserving both
      // the native joint centres and the native limb geometry.
    ...
    this.visualRoot.userData.visualRevision = 'prisma-rigged-humanoid-v7';
    this.visualRoot.userData.bodySilhouette = 'integrated-masculine-v3';
    this.visualRoot.userData.chestProfile = 'natural-pectoral-v3';
    this.visualRoot.userData.armSilhouette = 'native-authored-continuity-v5';
  ...
  #sculptReadableMasculineTorso() {
    ...
    geometry.userData.masculineArmProfile = 'native-authored-limbs-v2';
  }
