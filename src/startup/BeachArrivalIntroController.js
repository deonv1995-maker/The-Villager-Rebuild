...
export class BeachArrivalIntroController {
  ...
  #findWetSandStart(spawn) {
    ...
    if (
      ...
      // The established Day-1 point is already in the shallow wash on the
      // current island. Start there instead of moving the Ranger farther out
      // to sea before making him crawl back toward the same shoreline.
  ...
  #resolveInlandDirection(wetSand, spawn) {
    ...
    // If the authoritative spawn itself is the shallow-water start, use the
    // island centre only as a direction reference. The endpoint is still
    // sampled from the live terrain and never hard-coded to the centre.
