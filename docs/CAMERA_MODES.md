...
- the final third-person camera position is resolved through the shared world collision authority. A surface-anchored orbit preserves its horizontal follow distance and lifts only as much as needed for the complete sight line to clear the terrain skin; this prevents a cave lip from collapsing the camera onto the Ranger while still keeping the camera above cave openings. Once the Ranger anchor is genuinely underground, the camera switches to the volumetric solid-density query and shortens against cave walls/floors/roof instead of escaping the tunnel;
