...
RangerController remains authoritative for movement, collision and vertical position. It only asks the registered Sprout flight-assist provider to start or stop. While the provider reports active flight, Ranger vertical velocity is driven toward the centrally tuned ascent speed and existing horizontal movement/collision continues through the normal player traversal path. Releasing jump, landing, entering a cinematic, losing alliance or exhausting Sprout energy ends flight and normal gravity resumes.
...
The rocket shoes now read as **Sprout physically reconfigured around the Ranger's feet**, rather than unrelated equipment. They reuse the same authoritative Sprout cream shell, green panels, orange banding and cyan energy palette. Each foot unit carries a compact Sprout face-screen/eye cue, leaf-fin silhouette and hover-ring/thruster language so the pair reads as the companion split into two powered assemblies. Those shared colours come from SproutVisualAsset rather than a copied second palette.

The transformation is deliberately fast: the two assemblies begin as compact cyan transformation cores, split outward, unfold their shell/toe/ankle pieces, open the leaf fins and face panels, then ignite the hover rings and thrusters over **0.16 seconds**. The animation is presentation-only and does not delay or duplicate traversal authority.

While the rocket shoes are active, horizontal directional movement is **2.5× the established running speed**: the current 6-unit running speed becomes **15 world units per second** in flight. This boost is centralized in PlayerTraversalTuning and applies only while Sprout flight is active; ordinary walking and running remain unchanged.
...
For rocket-shoe flight, verify both keyboard and touch input: a quick second-jump tap must remain only a double jump; holding the second press should show the fast Sprout-to-shoes split/unfold/ignition sequence, with the cream/green/orange body language, face/eye cue, leaf fins and cyan hover hardware still visibly reading as Sprout. Once active, directional flight should feel substantially faster at exactly 2.5× running speed while remaining controllable, drain the existing Sprout gauge continuously, stop immediately on release, and force shutdown cleanly at zero energy. Confirm the shoes track both feet in third person and flight remains mechanically usable in first person without unhiding the body.
