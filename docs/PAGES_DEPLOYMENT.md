...
The production game/PWA contract still requires the **Vite `dist` artifact to be the final Pages deployment**, because Vite copies `public/manifest.webmanifest`, `public/sw.js`, and `public/icons/*` to the deployed site root and emits the production asset bundle expected by the installed PWA.

The repository-source Pages artifact is transiently visible while GitHub's branch-source deployment runs. The raw source entry therefore remains browser-runnable as a safety net: `index.html` loads runtime CSS such as `src/torch.css` and `src/sprout.css` directly, while `src/main.js` must not import CSS modules that a browser would reject when source files are served without Vite. This prevents the transient branch-source artifact from freezing at the static `FOUNDATION 0.3.8 · STARTING` shell even though production `dist` is still the authoritative deployment.
...
Do not add another competing production deployment path. If repository Pages settings are later switched to **GitHub Actions** as the sole source, remove the compatibility wait and retain the normal push-to-main production deployment. The raw-source startup safeguard may remain because it keeps the repository entry point standards-compliant and avoids coupling boot correctness to bundler-only CSS imports.
