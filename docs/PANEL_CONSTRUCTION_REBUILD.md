...
The Villager construction model is now moving from log-for-log structural inference to explicit semantic building modules.
...
Each wall also retains semantic owner/interior information and inward/outward normals. Rendering converts that structural orientation into world transforms; Ranger facing, camera yaw and saved mesh quaternions do not decide which side is inside.
...
Floor Panels create one full-cell standable collider and reuse `FloorSupportVisual` for uneven-ground supports/fill. The island terrain remains authoritative and is not permanently flattened or mutated for construction.
...
The existing compact build tray is reused rather than creating a competing interface. During this slice it exposes only:

- Floor;
- Wall;
- Close build mode.

The Log inventory row opens/closes build mode. Desktop `B` also enters/cycles Floor/Wall mode. `E` / `V` confirms a valid preview and `G` / Escape closes build mode.

Third person uses Ranger-relative placement. First person scores semantic slots against the centre-camera aim ray so the white reticle selects a structural target rather than relying on the Ranger body facing.
...
The replacement now has two layers of regression coverage:
...
2. `verify-panel-construction-runtime.mjs` protects the live Floor/Wall loop: material costs, preview validity, collision, per-building orientation, dependency-safe demolition/refunds and runtime restore.

Both are part of `npm run check`.

## Preserved systems

This slice intentionally preserves terrain/world generation, ecology, Ranger locomotion/camera, general inventory/crafting, tools and durability, combat, campfire behavior, common collision, PWA/install architecture and deployment architecture.
