...
Gameplay grounding deliberately uses the highest support sampled under the player's footprint so the controller does not clip into uneven terrain. The first Hero M grounding pass compensated from that controller support toward the walkable surface at the player's center. Physical-device review then exposed the remaining root cause: after Hero M's compact leg rig is retargeted, the visible boot soles can still sit above the mathematically correct presentation root even on flat ground.

The production presentation therefore has two presentation-only grounding stages. `HeroMPresentation` keeps the bounded center-support compensation used for uneven terrain. `HeroMVisibleSoleGroundingPresentation` then samples a small calibrated set of the actual skinned vertices at the bottom of both visible boots, after the current Hero M pose has been applied. While grounded and idle, it measures the lowest visible sole clearance against the exact walkable support beneath those sole samples and learns one bounded residual Y correction. That correction is applied only to Hero M's motion pivot and is retained through locomotion and airborne states so running and jumping do not acquire frame-by-frame vertical jitter.

The sole correction never raises the character, is capped at 0.42 m, ignores supports far below the current controller surface (for example a foot briefly projecting beyond a raised floor edge), and never changes the gameplay root, collision, terrain or jump physics.
