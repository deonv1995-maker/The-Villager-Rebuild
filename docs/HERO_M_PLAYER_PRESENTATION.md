...
### Ground-contact readability

A further physical-phone screenshot still read visually as floating even after the geometric fixes above. A production-asset runtime probe was added before changing the character again. It replays the real KayKit `Idle_A` against the real Hero M runtime body and measures the final posed bounds after the complete grounding stack. Across the sampled idle cycle the body bottom was already roughly 5.8–6.6 cm below a flat walkable surface, so lowering Hero M again would create real sinking rather than solve the remaining perception problem. A separate check of the Day-1 low-poly terrain found the analytical-versus-rendered surface difference stayed below roughly 1.7 cm in the sampled route, which was also too small to explain the device screenshot.

The remaining issue is therefore treated as contact readability rather than another controller-height problem. The old contact cue was one broad, very low-opacity disc centered under the Ranger root. At normal mobile camera distance, and especially on dark ground, it could disappear visually even though the boots were already planted. `HeroMVisibleSoleGroundingPresentation` now exposes two rendering-only ground-contact anchors from the actual animated Hero M foot bones, with their Y values resolved through the same walkable-support seam as the sole grounding. `CelestialShadowSystem` consumes those anchors and renders two compact, stronger ambient-occlusion contact layers directly beneath the visible feet. If Hero M is not ready or the active presentation does not provide both anchors, the previous centered contact cue remains as a safe fallback. All contact cues remain hidden in first person.

This foot-contact cue is deliberately a rendering seam. It does not move Hero M, the KayKit gameplay root, terrain, collision support, jump state or animation. The purpose is to make already-correct physical contact visually legible on a small phone display without introducing a second grounding authority.
