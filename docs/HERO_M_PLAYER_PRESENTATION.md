...
The sole correction is an absolute presentation offset, while every new measured clearance is a residual error after the previous correction has already been applied. The grounding loop therefore converts the residual error into a new absolute target by adding it to the current correction. It must never replace the existing correction with the residual measurement itself. That distinction is critical: treating a remaining gap as a fresh absolute offset causes the feedback loop to pull Hero M upward again and converge with a persistent visible hover.

The sole-support edge guard remains anchored to the walkable surface at the player's center rather than the max-footprint gameplay root. This keeps genuine downhill/shoreline support valid even when the collision root is substantially higher, while still rejecting a foot sample that projects far below the center support at the edge of a raised floor. The sole correction is capped at 0.68 m downward. It may recover upward when a later grounded pose is genuinely over-settled, but it is clamped so it can never raise Hero M above the uncorrected presentation baseline. It never changes the gameplay root, collision, terrain or jump physics.
...
A later phone screenshot provided a stronger diagnostic because both visible boots were still clearly separated from their two terrain contact cues. That exposed a fourth issue in the feedback math itself. The grounding pass measured clearance after the previous correction was already active, but then treated that remaining clearance as the next absolute correction. For example, if Hero M had already been lowered by 11 cm and still showed roughly 22.4 cm of residual clearance, the required target is about 34.6 cm downward including visual settle; replacing the existing correction with only the residual value instead pulls the presentation back upward. The production seam now accumulates the measured residual into the current absolute correction and uses the same rule through grounded idle, walk and run.
