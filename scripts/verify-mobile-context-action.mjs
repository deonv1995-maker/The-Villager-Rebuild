...
assert.match(assetPathsSource, /shovel: asset\('ui\/cosy\/icon-shovel\.svg'\)/, 'Shovel icon path must remain centralized');
assert.ok(fs.existsSync(new URL('../public/assets/ui/cosy/icon-shovel.svg', import.meta.url)), 'Shovel icon asset must exist in public assets');
...
for (const mode of ['raw', 'floor', 'frame', 'wall', 'stairs', 'roof', 'drop']) {
  ...
  assert.match(assetPathsSource, new RegExp(`${mode}: asset\\('ui/cosy/icon-build-${mode}\\.svg'\\)`), `${mode} must use a dedicated build icon asset`);
  assert.ok(
    fs.existsSync(new URL(`../public/assets/ui/cosy/icon-build-${mode}.svg`, import.meta.url)),
