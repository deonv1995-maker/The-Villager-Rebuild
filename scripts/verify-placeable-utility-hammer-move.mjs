...
assert(storage.getWorldEntries()[0]?.root?.isObject3D, 'Placed storage must expose its owned world root for hammer targeting');
runFrame();
...
assert(runtime.benchSystem.getWorldEntries()[0]?.root?.isObject3D, 'Crafting Bench must expose its owned world root for hammer targeting');
runFrame();
