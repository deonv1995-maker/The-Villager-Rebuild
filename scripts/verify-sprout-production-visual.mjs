...
const checks = [
  ...
  ['laser scanner is visible only while powered scanning state is active', scannerVisual.includes('const active = Boolean(powered && scanning)') && scannerVisual.includes('scannerVisual.visible = active') && scannerVisual.includes('if (!active) return') && scannerVisual.includes('sweep.rotation.y = elapsed * 3.15')],
  ...
  ['runtime forwards companion scanning state, selected target and existing terrain sampler into the scanner rendering layer', runtime.includes('ensureSproutScannerVisual') && runtime.includes('updateSproutScannerVisual') && runtime.includes('presentation.scanning') && runtime.includes('presentation.scanTarget') && runtime.includes('target: scanTarget') && runtime.includes('this.terrainHeightAt') && runtime.includes('terrainHeightAt: this.terrainHeightAt')],
