...
const checks = [
  ...
  ['visual includes face screen, expressive glow, twin leaf fins and concept 02 accent', asset.includes("name: 'sprout-face-screen'") && asset.includes("name: 'sprout-eye-left'") && asset.includes("name: 'sprout-expression-mouth'") && asset.includes("name: 'sprout-leaf-fin-left'") && asset.includes("name: 'sprout-leaf-fin-right'") && asset.includes("name: 'sprout-fin-02-mark'")],
  ['visual includes scanner, helper arms, lamp, multi-tool, back module and hover system', asset.includes("name: 'sprout-scanning-lens'") && asset.includes("'sprout-left-helper-arm'") && asset.includes("'sprout-right-helper-arm'") && asset.includes("name: 'sprout-utility-lamp'") && asset.includes("name: 'sprout-multitool-palm'") && asset.includes("name: 'sprout-removable-back-module'") && asset.includes("name: 'sprout-antigrav-ring'") && asset.includes("sprout-stabilizer-pod-")],
  ['visual animation is presentation-only and exposes powered/scanning states', asset.includes('export function updateSproutVisual') && asset.includes('powered = true') && asset.includes('scanning = false') && !asset.includes('inventory') && !asset.includes('gatherables') && !asset.includes('collision')],
  ['runtime replaces only the fallback presentation while preserving transform and visibility', runtime.includes('production.position.copy(existing.position)') && runtime.includes('production.quaternion.copy(existing.quaternion)') && runtime.includes('production.scale.copy(existing.scale)') && runtime.includes('production.visible = existing.visible') && runtime.includes('disposeFallbackPresentation(existing)')],
  ...
  ['documentation records production status, concept features, mobile budget and no third-party model dependency', docs.includes('production visual active') && docs.includes('approved hovering spherical concept') && docs.includes('Mobile budget') && docs.includes('Third-party license dependency: none')],
