...
const scannerDocs = read('docs/SPROUT_SCANNER_VISUAL.md');
...
const checks = [
  ...
  ['laser scanner effect is isolated in the rendering layer and installs idempotently', scannerVisual.includes('export function ensureSproutScannerVisual(root)') && scannerVisual.includes("scannerVisual.name = 'sprout-laser-scanner-visual'") && scannerVisual.includes('if (root.userData.sproutScannerVisual) return root.userData.sproutScannerVisual') && !scannerVisual.includes('inventory') && !scannerVisual.includes('gatherables') && !scannerVisual.includes('beamRange')],
  ['laser scanner renders a cone, holographic grid, contour rings and rotating ground sweep', scannerVisual.includes('new THREE.ConeGeometry(BEAM_RADIUS, BEAM_HEIGHT, 24, 1, true)') && scannerVisual.includes("grid.name = 'sprout-laser-scan-grid'") && scannerVisual.includes('new THREE.LineSegments(createRayGeometry(), gridMaterial)') && scannerVisual.includes('new THREE.TorusGeometry(BEAM_RADIUS * 1.01, 0.018, 5, 36)') && scannerVisual.includes("sweep.name = 'sprout-laser-scan-sweep'")],
  ['targeted scanner projects a terrain-conforming laser grid and detected-item marker from shared terrain height samples', scannerVisual.includes('TERRAIN_GRID_DIVISIONS = 6') && scannerVisual.includes("terrainGrid.name = 'sprout-laser-terrain-grid'") && scannerVisual.includes("targetMarker.name = 'sprout-laser-detected-item-marker'") && scannerVisual.includes("typeof terrainHeightAt === 'function'") && scannerVisual.includes('terrainHeightAt(ax, az) + TERRAIN_GRID_LIFT') && scannerVisual.includes('terrainHeightAt(target.x, target.z)') && scannerVisual.includes('root.worldToLocal(terrainPointLocal)')],
  ['laser scanner originates at the authored scanning lens and can aim at the selected world target', scannerVisual.includes('const scannerLens = root.userData.scannerLens') && scannerVisual.includes('scannerLens.getWorldPosition(lensWorld)') && scannerVisual.includes('root.worldToLocal(lensLocal)') && scannerVisual.includes('targetWorld.set(target.x, target.y + 0.08, target.z)') && scannerVisual.includes('beamPivot.quaternion.setFromUnitVectors(DOWN_AXIS, scanDirection)')],
  ['laser scanner stays mobile-lightweight with additive unlit materials and no dynamic lights or shaders', scannerVisual.includes('THREE.AdditiveBlending') && scannerVisual.includes('new THREE.MeshBasicMaterial') && scannerVisual.includes('new THREE.LineBasicMaterial') && scannerVisual.includes('depthWrite: false') && !scannerVisual.includes('PointLight') && !scannerVisual.includes('ShaderMaterial')],
  ['laser scanner is visible only while powered scanning state is active', scannerVisual.includes('const active = Boolean(powered && scanning)') && scannerVisual.includes('scannerVisual.visible = active') && scannerVisual.includes('if (!active) return') && scannerVisual.includes('sweep.rotation.y = elapsed * (3.15 + strength * 3.4)')],
  ...
  ['runtime forwards companion scanning state, target, conditional terrain projection and signal strength into the scanner rendering layer', runtime.includes('ensureSproutScannerVisual') && runtime.includes('updateSproutScannerVisual') && runtime.includes('presentation.scanning') && runtime.includes('presentation.scanTarget') && runtime.includes('presentation.scanTerrainProjection') && runtime.includes('presentation.scanIntensity') && runtime.includes('target: scanTarget') && runtime.includes('terrainHeightAt: scanTerrainProjection ? this.terrainHeightAt : null') && runtime.includes('signalStrength: scanIntensity')],
  ...
  ['scanner documentation preserves lens-origin sequencing, terrain projection, detected-item presentation and mobile boundary', scannerDocs.includes('active presentation effect') && scannerDocs.includes('scanning lens') && scannerDocs.includes('scan-lock pause') && scannerDocs.includes('terrain-conforming cyan wire grid') && scannerDocs.includes('detected-item ring/crosshair') && scannerDocs.includes('presentation only') && scannerDocs.includes('Mobile rendering budget') && scannerDocs.includes('Rendering code must not introduce a second scan radius')],
