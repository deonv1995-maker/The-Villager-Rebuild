...
const checks = [
  ...
  ['visual version 3 carries one authored 75 percent presentation scale', asset.includes('root.userData.visualVersion = 3') && asset.includes('const PRESENTATION_SCALE = 0.75') && asset.includes('root.userData.presentationScale = PRESENTATION_SCALE')],
  ...
  ['helper arms have presentation-only delayed vertical inertia independent of companion movement authority', asset.includes('leftArm.position.y = (leftArm.userData.baseY ?? -0.02)') && asset.includes('rightArm.position.y = (rightArm.userData.baseY ?? -0.02)') && asset.includes('elapsed * 2.05 - 0.56') && asset.includes('elapsed * 2.05 - 0.74')],
  ['three stabilizer assemblies and hover rings receive smooth asynchronous vertical motion', asset.includes('const stabilizerPods = []') && asset.includes('sprout-stabilizer-root-') && asset.includes('sprout-stabilizer-ring-') && asset.includes('const irregular = Math.sin') && asset.includes('hoverRing.position.y =') && asset.includes('hoverInnerRing.position.y =')],
  ...
  ['documentation records production status, concept-reference refinement, mobile budget and no third-party model dependency', docs.includes('production visual active') && docs.includes('approved hovering spherical concept') && docs.includes('Refinement pass (visual version 3)') && docs.includes('Mobile budget') && docs.includes('Third-party license dependency: none')],
