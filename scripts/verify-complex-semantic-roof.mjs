...
const liveHighRoof = [...liveSteppedRoofStructure.grid.roofZones.values()]
  .find(zone => zone.storey === 1);
const liveLeftLowerRoof = [...liveSteppedRoofStructure.grid.roofZones.values()]
  .find(zone => (
    zone.storey === 0 &&
    zone.cellKeys.includes(panelCellKey({ x: -1, z: 0, storey: 0 }))
  ));
assert.ok(liveHighRoof && liveLeftLowerRoof);
assert.equal(liveSteppedRoofStructure.grid.removeRoofZone(liveHighRoof.key), true);
assert.equal(
  liveSteppedRoofStructure.grid.removeWall(steppedUpperWallKeys.west),
  false,
  'An upper Wall used as the lower Roof abutment must stay protected after the high Roof is removed'
);
assert.equal(liveSteppedRoofStructure.grid.removeRoofZone(liveLeftLowerRoof.key), true);
assert.equal(
  liveSteppedRoofStructure.grid.removeWall(steppedUpperWallKeys.west),
  true,
