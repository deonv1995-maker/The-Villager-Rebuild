...
const checks = [
  ...
  ['roof construction preserves its gable or mono-pitch form', persistence.includes('roofForm: entry.roofForm') && persistence.includes('roofForm: saved.roofForm ?? null') && persistence.includes('highEdge: saved.highEdge ?? null')],
  ...
  ['Continue runs the save-only FRAME-cell wall-facing recovery before restoring wall variants', persistence.includes('system.recoverRestoredFacing()') && persistence.indexOf('system.recoverRestoredFacing()') < persistence.indexOf('system.customize(panel.id, panel.variant)')],
