...
const checks = [
  ...
  ['global tunneling snapshot is added to the shared save state', saveController.includes('state.tunneling = this.game.island?.explorationPois?.captureState?.() ?? null') && !saveController.includes('state.caveMining =')],
  ['global tunneling restores before Ranger placement', saveController.includes('restoreState?.(record.state.tunneling)') && saveController.indexOf('restoreState?.(record.state.tunneling)') < saveController.indexOf('restoreGameState(this.game, record.state)')],
  ...
  ['Ranger save position is fully 3D for cave-safe Continue', persistence.includes('position: { x: position.x, y: position.y, z: position.z }') && persistence.includes('Number.isFinite(savedY) ? savedY : null')],
  ['legacy 2D cave saves recover to a solid surface instead of an underground support', persistence.includes('recoverLegacyCavePlacement(game, x, z)') && persistence.includes('hasTunnelingActivityAt?.(x, z)') && persistence.includes('solidBelow') && persistence.includes('clearAbove')],
