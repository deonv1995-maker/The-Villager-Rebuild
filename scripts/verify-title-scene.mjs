...
const checks = [
  ...
  ['title bowsprit is structurally joined to the bow', shipVisual.includes('createBeamBetween') && shipVisual.includes("name: 'title-ship-bowsprit'") && shipVisual.includes("name: 'title-ship-bowsprit-knee'") && shipVisual.includes("bowspritCollar.name = 'title-ship-bowsprit-collar'")],
  ...
  ['title sail stays forward of the mast and carries a wind-filled bow', config.includes('sailMastClearance: 0.34') && config.includes('sailBowCalm: 0.42') && config.includes('sailBowStorm: 0.68') && shipVisual.includes('TITLE_SCENE.sailMastClearance') && shipVisual.includes('const windBow = THREE.MathUtils.lerp') && shipVisual.includes('interior * windBow')],
  ...
  ['arrival prone and crawl poses sample rising terrain under the full body', arrivalIntro.includes('#proneGroundOffsetAt') && arrivalIntro.includes('CRAWL_BODY_SAMPLE_DISTANCES') && arrivalIntro.includes('CRAWL_BODY_HALF_WIDTH') && arrivalIntro.includes('Crawl_SURFACE_PADDING') === false && arrivalIntro.includes('CRAWL_SURFACE_PADDING') && arrivalIntro.includes('modelYOffset: this.#proneGroundOffsetAt(this.wetSand.x, this.wetSand.z, PRONE_BASE_CLEARANCE)') && arrivalIntro.includes('modelYOffset: this.#proneGroundOffsetAt(x, z, crawlBaseClearance)')],
