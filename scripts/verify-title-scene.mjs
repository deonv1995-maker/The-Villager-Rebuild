...
const checks = [
  ...
  ['New Game opens a named-profile setup before the intro sequence', titleScene.includes('beginNewGameSetup({ onConfirm } = {})') && titleScene.includes('Name your Ranger') && titleScene.includes('await this.playIntro()')],
  ...
  ['intro duration follows wall-clock time instead of stretching indefinitely on low-FPS devices', titleScene.includes('this.introStartedAt = globalThis.performance?.now?.() ?? null') && titleScene.includes('(now - this.introStartedAt) / 1000')],
  ['intro handoff catches synchronous and asynchronous startup failures instead of leaving the transition cover stuck', titleScene.includes("console.error('[TITLE HANDOFF]'") && titleScene.includes("typeof handoff.then === 'function'") && titleScene.includes('this.#handleHandoffError(error)')],
  ['gameplay boot catches title disposal and save-store setup failures inside the shared startup error path', main.includes("async function bootGameplay") && main.includes("try {\n    titleScene?.dispose({ keepTransition: true });\n    const saveStore = new SaveGameStore")],
  ...
  ['arrival intro uses wall-clock timing so low frame rate cannot leave gameplay hidden indefinitely', arrivalIntro.includes('this.introStartedAt = globalThis.performance?.now?.() ?? null') && arrivalIntro.includes('(now - this.introStartedAt) / 1000') && arrivalIntro.includes('if (this.introElapsed >= ARRIVAL_DURATION)') && !arrivalIntro.includes('this.phaseElapsed += dt')],
  ['arrival intro has an independent fail-open watchdog that always releases hidden-HUD state', arrivalIntro.includes('ARRIVAL_WATCHDOG_GRACE_SECONDS') && arrivalIntro.includes("() => this.#complete('watchdog')") && arrivalIntro.includes("document.body.classList.remove('arrival-intro-active')") && arrivalIntro.includes('this.player.endCinematic(this)') && arrivalIntro.includes("console.warn('[ARRIVAL] Unable to release Ranger cinematic ownership'")],
  ...
  ['main boots title before gameplay and defers profile creation until the confirmed intro handoff', main.includes('new TitleSceneApp') && main.includes('onNewGameRequest: () => titleScene.beginNewGameSetup') && main.includes('const profile = profileName ? profileStore.create(profileName) : null') && main.includes('async function bootGameplay')],
