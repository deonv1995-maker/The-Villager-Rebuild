...
const checks = [
  ...
  ['gameplay continuation uses the shared falling-star trail with a bright tapered plume, accent streaks and sparks', crashSite.includes('createFallingStarTrailVisual') && crashSite.includes("outerName: 'sprout-gameplay-plasma-tail'") && crashSite.includes("innerName: 'sprout-gameplay-inner-plasma-tail'") && fallingStarTrail.includes('mesh.rotation.z = Math.PI') && fallingStarTrail.includes('light-streak-') && fallingStarTrail.includes('spark-particles') && crashSite.includes("this.smoke.name = 'sprout-blue-impact-smoke'")],
  ...
  ['fresh impact keeps ejecta downrange while approach scours trail back along the incoming flight path', crashSite.includes('const baseAngle = Math.atan2(direction.y, direction.x)') && crashSite.includes('const approachScarDirection = direction.clone().multiplyScalar(-1)') && crashSite.includes('const centerX = approachScarDirection.x * distanceBack') && crashSite.includes('const centerZ = approachScarDirection.y * distanceBack') && crashSite.includes('sprout-impact-ejecta-') && crashSite.includes('sprout-impact-ejecta-scour-')],
